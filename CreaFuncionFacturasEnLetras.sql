...
-- Function: decenas(numeric, numeric)

-- DROP FUNCTION decenas(numeric, numeric);

CREATE OR REPLACE FUNCTION decenas(
    prdigito numeric,
    prunidad numeric)
  RETURNS character AS
$BODY$ 
...
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION decenas(numeric, numeric)
  OWNER TO postgres;

-- centenas


CREATE OR REPLACE FUNCTION centenas(
    prdigito numeric,
    prdecena numeric,
    prunidad numeric)
  RETURNS character AS
...
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION centenas(numeric, numeric, numeric)
  OWNER TO postgres;

-- uniddes


CREATE OR REPLACE FUNCTION unidades(prvalor numeric)
  RETURNS character AS
$BODY$ 
DECLARE strVlrLetras character(20);
BEGIN
...
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION unidades(numeric)
  OWNER TO postgres;
