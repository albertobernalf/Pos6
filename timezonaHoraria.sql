...
select documento_id, * from sitios_dependencias WHERE DOCUMENTO_ID IS NOT NULL
update sitios_dependencias set "tipoDoc_id" =null, documento_id=null, consec=0,"fechaLiberacion"=null, "fechaOcupacion"=null where id=11
select * from sitios_historialdependencias order by id
	-- estoy en la dependencia 11
...
SELECT sub.id id ,sub.nombre nombre 
	 ...
	 Where sed."sedesClinica_id" = '1' and sed."sedesClinica_id" = sub."sedesClinica_id" and 
	 sed.id = sub."serviciosSedes_id"  and sed.id = '7'  -- and sub."serviciosSedes_id" = sed.id
	 and serv.id = sed.servicios_id
...
select * from sitios_subserviciossedes
	...
	select "serviciosSedes_id",* from sitios_dependencias where id in (10,21,17)

	1	10
	2	17
	7	21
	
select "serviciosIng_id", "serviciosActual_id", "serviciosSalida_id","dependenciasIngreso_id",documento_id,* from admisiones_ingresos;
...
select * from sitios_serviciosSedes;
select * from sitios_subserviciosSedes;
select * from sitios_dependencias where"serviciosSedes_id" =7
select * from usuarios_usuarios where id ('24','25'); -- 24 en "AMBULAT-201"  25 en "Ambulat-202"
                                                             -- 24 luis ernesto /7 --25 alejandro
-- ESTA ES LA BUENA
