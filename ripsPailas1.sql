...
SELECT * FROM rips_ripsprocedimientos where "ripsTransaccion_id" =395
SELECT * FROM rips_ripstransaccion where transaccion_id=394
	select * from facturacion_facturaciondetalle where facturacion_id=142
-- ripsprocedimeintos
...
INSERT INTO rips_ripsprocedimientos ("codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion",
	...
	"finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id", "tipoDocumentoIdentificacion_id",
	...
	autdet."numeroAutorizacion", usu.documento, facdet."valorTotal", '00', fac.id, row_number()  OVER(ORDER BY facdet.id)  AS consecutivo, now(), 
	(select max(diag4.id) from clinico_diagnosticos diag4 where  diag4.id = i."dxComplicacion_id"), 
	(select  max(diag1.id) from clinico_historialdiagnosticos histdiag1, clinico_diagnosticos diag1 
	where histdiag1.historia_id = his.id and histdiag1."tiposDiagnostico_id" = '1'), 
	(select max(diag3.id) from clinico_historialdiagnosticos histdiag3, clinico_diagnosticos diag3
	where histdiag3.historia_id = his.id and histdiag3."tiposDiagnostico_id" = '2'), exa.id, serv.id,'5'  , 
	final.id, gru.id, mod.id, tipdocrips.id, '1', ingreso.id, detrips.id, facdet."consecutivoFactura", '4',
	...
	where i."tipoDoc_id" = pagos."tipoDoc_id" and i.documento_id = pagos.documento_id and i.consec = pagos.consec and
	pagos."formaPago_id" = formapago.id and ripsmoderadora."codigoAplicativo" = cast(formapago.id as text)), '323232','A',
	'9999' 
	FROM sitios_sedesclinica sed 
	inner join facturacion_facturacion fac ON (fac."sedesClinica_id" = sed.id) 
	inner join facturacion_facturaciondetalle facdet ON (facdet.facturacion_id = fac.id and facdet."examen_id" is not null and (facdet.anulado = 'N' or facdet.anulado = 'R') and "tipoRegistro" = 'SISTEMA')
	...
	left join autorizaciones_autorizaciones  aut on (aut.historia_id = his.id)
	left join autorizaciones_autorizacionesdetalle autdet on (autdet.autorizaciones_id = aut.id and autdet.examenes_id = facdet.examen_id) 
	where sed.id = '1' and e.id = '68' and fac.id = '142'
            
select * from clinico_historiaexamenes where historia_id in ('1488','1489')
select * from facturacion_facturacion where id =142
select * from clinico_historia where documento_id='9' and "tipoDoc_id" = '1' 	 and "consecAdmision" =1
select * from facturacion_facturaciondetalle where facturacion_id =142
