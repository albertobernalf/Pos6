...
comando = 'select  h.id HistoriaId, h.folio folioId, h.fecha fechaFolio, h."tiposFolio_id" tipoFolio from clinico_historia h where h.documento_id = ' + "'" + str('1') + "'" + ' order by id'
curt.execute(comando)
print(comando)
...
for i in range(0, len(folios)):
   ...
   pdf.cell(1, 1, '__________________________________________________________________________________________________________________________________________', 0, 0, 'L')
   ...
   linea = linea + 1
   pdf.ln(1)
   pdf.cell(1, 10,'__________________________________________________________________________________________________________________________________________', 0, 0, 'L')
   ...
   for l in range(0, len(historia)):
       ...
       for antibioticos, apache2 ,examenFisico, hipotension, ingestaAlcohol, irritacion,justificacion, leucopenia,llenadoCapilar, monitoreo, movilidadLimitada, nauseas,  neurologia, notaAclaratoria , pulsos, retiroPuntos,  riesgoHemodinamico, riesgoVentilatorio, riesgos, textoNotaAclaratoria, tratamiento, trombocitopenia, vomito  in curh.fetchall():
           rasgosHistoria.append(
               {'antibioticos': antibioticos, 'apache2': apache2,'examenFisico':examenFisico,'hipotension':hipotension,
                'ingestaAlcohol': ingestaAlcohol, 'irritacion': irritacion, 'justificacion': justificacion,'leucopenia': leucopenia,
                'llenadoCapilar': llenadoCapilar, 'monitoreo': monitoreo, 'movilidadLimitada': movilidadLimitada, 'nauseas': nauseas,
                'neurologia': neurologia, 'notaAclaratoria': notaAclaratoria, 'pulsos': pulsos,'retiroPuntos': retiroPuntos,
                'riesgoHemodinamico': riesgoHemodinamico, 'riesgoVentilatorio': riesgoVentilatorio, 'riesgos': riesgos,    'textoNotaAclaratoria': textoNotaAclaratoria,
       ...
       miConexionr = psycopg2.connect(host="192.168.79.133", database="vulner6", port="5432", user="postgres",    password="123456")
       curr = miConexionr.cursor()

       comando = 'SELECT revision.nombre sistema, hist.observacion observacion FROM clinico_historiarevisionsistemas hist INNER JOIN clinico_revisionsistemas revision  ON (revision.id = hist."revisionSistemas_id") WHERE hist.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       miConexiond = psycopg2.connect(host="192.168.79.133", database="vulner6", port="5432", user="postgres",    password="123456")
       curd = miConexiond.cursor()

       comando = 'SELECT antecedente.nombre antecedente, histAntecedente.descripcion descripcion FROM clinico_historialantecedentes histAntecedente INNER JOIN clinico_tiposantecedente antecedente  ON (antecedente.id = histAntecedente."tiposAntecedente_id") WHERE histAntecedente.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       miConexione = psycopg2.connect(host="192.168.79.133", database="vulner6", port="5432", user="postgres",    password="123456")
       cure = miConexione.cursor()

       comando = 'SELECT to_char(signos.fecha, ' + "'" + str('YYYY-MM-DD HH:MM') + "') fecha" + ' ,signos."frecCardiaca", signos."frecRespiratoria", signos."tensionADiastolica", signos."tensionASistolica", 	signos."tensionAMedia", signos.temperatura, signos.saturacion, signos.glucometria, signos.glasgow, signos.apache, signos.pvc, 	signos.cuna, signos.ic, signos."glasgowOcular", signos."glasgowVerbal", signos."glasgowMotora", signos.observacion FROM clinico_historiasignosvitales  signos WHERE signos.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       for fecha, frecCardiaca, frecRespiratoria, tensionADiastolica , tensionASistolica, tensionAMedia , temperatura, saturacion, glucometria, glasgow, apache,pvc, cuna, ic ,glasgowOcular, glasgowVerbal , glasgowMotora , observacion in cure.fetchall():
           signosVitales.append(
               {'fecha': fecha, 'frecCardiaca': frecCardiaca,'frecRespiratoria':frecRespiratoria,'tensionADiastolica':tensionADiastolica,'tensionASistolica': tensionASistolica, 'tensionAMedia':tensionAMedia,'temperatura':temperatura,'saturacion':saturacion ,'glucometria':glucometria, 'glasgow':glasgow, 'apache':apache, 'pvc':pvc, 'cuna':cuna, 'ic':ic,'glasgowOcular':glasgowOcular ,'glasgowVerbal':glasgowVerbal,'glasgowMotora':glasgowMotora, 'observacion':observacion})
       miConexione.close()
       ...
       comando = 'SELECT to_char(notas."fechaRegistro", ' + "'" + str('YYYY-MM-DD HH:MM') + "') fecha" + ' ,notas.observaciones nota  FROM clinico_historialnotasenfermeria  notas WHERE notas.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       for l in range(0, len(laboratorios)):
           ...
           comando = 'select rasgos.nombre rasgo, rasgos.unidad unidad, rasgos.minimo minimo , rasgos.maximo maximo,histresul.valor valor FROM clinico_historiaresultados histresul INNER JOIN 	clinico_historiaexamenes histExa on (histExa.id=histresul."historiaExamenes_id" and histExa."codigoCups" = ' + "'" +  str(laboratorios[0 + l]['codigoCups']) + "'" + ') INNER JOIN clinico_historia historia on (historia.id = histExa.historia_id ) INNER JOIN clinico_examenesrasgos rasgos ON (rasgos.id=histresul."examenesRasgos_id") where historia.id = ' + "'"  + str(folios[0+i]['HistoriaId']) + "'"
       ...
       comando ='select h."codigoCups" codigoCups, e.nombre nombre, h.cantidad cantidad, h.observaciones observaciones from clinico_historiaexamenes h, clinico_examenes    e, clinico_tiposexamen t  where h."tiposExamen_id" = t.id and t.nombre like(' + "'" + '%RAD%' + "'" + ') and e."codigoCups" = h."codigoCups" and h.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       for l in range(0, len(radiologia)):
           ...
           comando = 'select rasgos.nombre rasgo, rasgos.unidad unidad, rasgos.minimo minimo , rasgos.maximo maximo,histresul.valor valor FROM clinico_historiaresultados histresul INNER JOIN 	clinico_historiaexamenes histExa on (histExa.id=histresul."historiaExamenes_id" and histExa."codigoCups" = ' + "'" +  str(radiologia[0 + l]['codigoCups']) + "'" + ') INNER JOIN clinico_historia historia on (historia.id = histExa.historia_id ) INNER JOIN clinico_examenesrasgos rasgos ON (rasgos.id=histresul."examenesRasgos_id") where historia.id = ' + "'"  + str(folios[0+i]['HistoriaId']) + "'"
       ...
       comando ='select h."codigoCups" codigoCups, e.nombre nombre, h.cantidad cantidad, h.observaciones observaciones from clinico_historiaexamenes h, clinico_examenes    e, clinico_tiposexamen t  where h."tiposExamen_id" = t.id and t.nombre like(' + "'" + '%TERAP%' + "'" + ') and e."codigoCups" = h."codigoCups" and h.historia_id = ' + str(folios[0+i]['HistoriaId'])
       ...
       for l in range(0, len(terapias)):
           ...
           comando = 'select rasgos.nombre rasgo, rasgos.unidad unidad, rasgos.minimo minimo , rasgos.maximo maximo,histresul.valor valor FROM clinico_historiaresultados histresul INNER JOIN 	clinico_historiaexamenes histExa on (histExa.id=histresul."historiaExamenes_id" and histExa."codigoCups" = ' + "'" +  str(terapias[0 + l]['codigoCups']) + "'" + ') INNER JOIN clinico_historia historia on (historia.id = histExa.historia_id ) INNER JOIN clinico_examenesrasgos rasgos ON (rasgos.id=histresul."examenesRasgos_id") where historia.id = ' + "'"  + str(folios[0+i]['HistoriaId']) + "'"
       ...
       for especialidadConsulta, plantaConsulta, diagnostico, descripcionConsulta, especialidadConsultada, plantaConsultada, respuestaConsulta in curx.fetchall():
           interConsultas.append(
               {'especialidadConsulta': especialidadConsulta, 'plantaConsulta': plantaConsulta, 'diagnostico': diagnostico, 'descripcionConsulta': descripcionConsulta,'especialidadConsultada':especialidadConsultada, 'plantaConsultada':plantaConsultada,'respuestaConsulta':respuestaConsulta})
       miConexionx.close()
       ...
       for dosis, medida, via, suministro, cantidad, diasTratamiento in curc.fetchall():
           medicamentos.append(
               {'dosis': dosis, 'medida': medida, 'via': via, 'suministro': suministro,'cantidad':cantidad, 'diasTratamiento':diasTratamiento})
       miConexionc.close()
       ...
       for tipoDiag, consecutivo, nombreDiagnostico, observaciones in curt.fetchall():
           diagnosticos.append(
               {'tipoDiag': tipoDiag, 'consecutivo': consecutivo, 'nombreDiagnostico': nombreDiagnostico, 'observaciones': observaciones})
       miConexiont.close()
       ...
       comando = 'SELECT medicos."registroMedico", planta.nombre plantaNombre, usu."tipoDoc_id", usu.documento 	FROM clinico_historia historia INNER JOIN planta_planta planta ON (planta.id = historia."usuarioRegistro_id") INNER JOIN clinico_medicos medicos ON (medicos.id = historia."usuarioRegistro_id") INNER JOIN usuarios_usuarios usu ON (usu.id = historia."usuarioRegistro_id")	WHERE historia.id = ' + "'" + str(folios[0+i]['HistoriaId']) + "'"
       curx.execute(comando)
       ...
       for registroMedico, plantaNombre, tipoDoc_id, documento in curx.fetchall():
            registroMed.append({'registroMedico': registroMedico, 'plantaNombre': plantaNombre, 'tipoDoc_id': tipoDoc_id, 'documento': documento})
       miConexionx.close()
