...
SELECT histoexa.id examId ,'INGRESO' tipoIng, i.id||'-INGRESO' , tp.nombre tipoDoc,u.documento documento,u.nombre nombre,
	...
	and i."serviciosActual_id" = sd.id AND dep.disponibilidad = 'O' AND i."salidaDefinitiva" = 'N'
	...
	FROM admisiones_ingresos i
	INNER JOIN usuarios_usuarios u ON (u."tipoDoc_id" = i."tipoDoc_id" and u.id = i."documento_id")
	INNER JOIN sitios_dependencias dep ON (dep."sedesClinica_id" = i."sedesClinica_id" AND dep.disponibilidad = 'O' and dep.id = i."dependenciasActual_id" )
	INNER JOIN usuarios_tiposDocumento tp ON (tp.id = u."tipoDoc_id")
	INNER JOIN sitios_dependenciastipo deptip ON ( deptip.id = dep."dependenciasTipo_id") 
	INNER JOIN clinico_Diagnosticos diag on (diag.id = i."dxActual_id")
	INNER JOIN sitios_serviciosSedes sd ON (sd."sedesClinica_id" = i."sedesClinica_id" and sd.id = i."serviciosActual_id" and  sd.id = dep."serviciosSedes_id"  )
	INNER JOIN clinico_servicios ser ON (ser.id=sd.servicios_id and ser.id = sd.servicios_id   )
	INNER JOIN clinico_tiposexamen tipoExa ON (1=1)
	INNER JOIN  clinico_examenes exam ON ( exam."TiposExamen_id" = tipoExa.id)
	INNER JOIN clinico_historiaexamenes histoexa on (histoexa."tiposExamen_id" = tipoExa.id and  histoexa."tiposExamen_id" = exam."TiposExamen_id" and Histoexa."codigoCups" = exam."codigoCups")
	INNER JOIN clinico_historia historia ON (historia.id  = histoexa.historia_id AND   historia."tipoDoc_id" =  u."tipoDoc_id" AND historia.documento_id = u.id AND historia."consecAdmision" = i.consec)
	LEFT JOIN clinico_estadoexamenes estadosExam ON (estadosExam.id = histoexa."estadoExamenes_id" )
	WHERE  i."sedesClinica_id" = '1'  AND  i."salidaDefinitiva" = 'N'  and i."fechaSalida" is null AND estadosExam.nombre = 'ORDENADO'
